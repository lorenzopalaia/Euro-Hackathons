-- Crea la tabella hackathons (post-migration, senza campo location)
CREATE TABLE hackathons (
    id UUID DEFAULT gen_random_uuid() PRIMARY KEY,
    name TEXT NOT NULL,
    city TEXT,
    country_code TEXT, -- ISO 3166-1 alpha-2 code (e.g., 'GB', 'IT', 'US')
...
BEGIN
        RETURN QUERY 
        SELECT 
                h.id, h.name, h.city, h.country_code,
...
COMMENT ON COLUMN hackathons.city IS 'Nome della città dove si svolge l''hackathon (es: London, Berlin)';
COMMENT ON COLUMN hackathons.country_code IS 'Codice paese ISO 3166-1 alpha-2 (es: GB, IT, DE)';
